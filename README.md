...
If the callback throws an error or returns a falsy value, auth will fail with generic 401 Invalid Credentials error. (Throw a APIError to return specific error code and message, see below)
...
- The OnLinkAccount should return info to be stored in Account table, if it throw errors the login will fail with generic 401 Invalid Credentials error.

> If the error you throw is a instance of APIError like `import { APIError } from "@better-auth/core/error"`, the error returned will be the one you threw instead of the generic 401 Invalid Credentials error, so this way you can return a more specific error code and message to the user if needed. This works in the plugin config callback and the other three result callbacks (onSignIn, onSignUp and onLinkAccount).
